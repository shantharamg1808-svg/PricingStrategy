...
import { usePricingStore } from './GlobalPricingStore';
import { 
  Calculator, 
  TrendingUp, 
  TrendingDown, 
...
const COMMON_DURATIONS = [
  { days: 0.75, count: 129 }, { days: 1.0, count: 306 }, { days: 1.5, count: 19 },
  { days: 2.0, count: 255 }, { days: 2.5, count: 20 }, { days: 3.0, count: 338 },
  { days: 4.0, count: 144 }, { days: 5.0, count: 54 }, { days: 6.0, count: 36 },
  { days: 7.0, count: 27 }, { days: 8.0, count: 18 }
];

const LOCATIONS = [
  { name: 'Nandi Hills', distance: 65, days: 1 }, { name: 'Kolar', distance: 70, days: 1 },
  { name: 'Sravanabelagola', distance: 81, days: 1 }, { name: 'Lepakshi', distance: 125, days: 1 },
  { name: 'Shivanasamudra', distance: 130, days: 1 }, { name: 'BR Hills', distance: 195, days: 2 },
  { name: 'Gopalaswamy', distance: 195, days: 2 }, { name: 'Hassan', distance: 200, days: 2 },
  { name: 'Belur', distance: 200, days: 2 }, { name: 'Halebidu', distance: 200, days: 2 },
  { name: 'Kabini', distance: 210, days: 2 }, { name: 'Bandipur', distance: 210, days: 2 },
  { name: 'Chikmagalur', distance: 245, days: 3 }, { name: 'Coorg (Madikeri)', distance: 250, days: 3 },
  { name: 'Wayanad', distance: 270, days: 3 }, { name: 'Gandikota', distance: 290, days: 2 },
  { name: 'Belum Caves', distance: 290, days: 2 }, { name: 'Sringeri', distance: 335, days: 3 },
  { name: 'Kudremukh', distance: 335, days: 3 }, { name: 'Hampi', distance: 395, days: 3 },
  { name: 'Badami', distance: 395, days: 3 }, { name: 'Udupi', distance: 400, days: 3 },
  { name: 'Manipal', distance: 400, days: 4 }, { name: 'Jog Falls', distance: 414, days: 4 },
  { name: 'Gokarna', distance: 490, days: 4 }, { name: 'Murudeshwar', distance: 490, days: 4 },
  { name: 'Munnar', distance: 535, days: 5 }, { name: 'Alleppey', distance: 535, days: 5 },
  { name: 'Goa', distance: 575, days: 5 }, { name: 'Kanyakumari', distance: 660, days: 5 }
];
...
// --- APPLY PRICE MODIFIER HELPER ---
function applyPriceModifier(basePrice, globalModifier) {
  const modifier = 1 + (globalModifier / 100);
  return basePrice * modifier;
}

// --- MAIN DASHBOARD APP ---
export default function FinancialStrategySimulator() {
  const { pricingState } = usePricingStore();
  const globalModifier = Number(pricingState.globalModifier) || 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col">
      <div className="p-4 md:p-8 max-w-screen-2xl mx-auto w-full flex flex-col gap-6">
        
        {/* Header with Global Modifier Display */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <h2 className="text-2xl font-bold text-slate-800 mb-4">Revenue Projection Engine</h2>
          <p className="text-slate-600 mb-6">Model different monthly booking scenarios and compare financial outcomes. All prices automatically reflect global modifiers.</p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Total Monthly Bookings</label>
            ...
            {globalModifier !== 0 && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                <p className="text-[9px] font-bold text-emerald-600 uppercase mb-2">Global Price Adjustment</p>
                <p className="text-2xl font-bold text-emerald-700">{globalModifier > 0 ? '+' : ''}{globalModifier.toFixed(1)}%</p>
                <p className="text-[10px] text-emerald-600 mt-1">Applied to all price calculations</p>
              </div>
            )}
          </div>
          ...
          {globalModifier !== 0 && (
            <div className="mt-6 p-4 bg-slate-50 border border-slate-200 rounded-lg">
              <p className="text-[10px] font-bold text-slate-500 uppercase mb-2 flex items-center gap-2">
                <Info size={14} /> Global Price Impact
              </p>
              <p className="text-xs text-slate-700 leading-relaxed">
                The global pricing adjustment from the Multi-Vehicle Dashboard is actively being applied to all price calculations throughout this projection engine. This ensures consistent pricing across your entire platform.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-800 rounded-2xl p-6 text-white">
            <h3 className="text-sm font-bold text-slate-400 uppercase mb-2">Scenario A (Existing)</h3>
            <p className="text-3xl font-bold mb-2">Base Strategy</p>
            <p className="text-xs text-slate-400">Fixed 140/320/500/620 KM pricing model</p>
          </div>

          <div className="bg-gradient-to-br from-[#f04343] to-[#d42d2d] rounded-2xl p-6 text-white">
            <h3 className="text-sm font-bold text-white/80 uppercase mb-2">Scenario B (Custom)</h3>
            <p className="text-3xl font-bold mb-2">Projected Strategy</p>
            <p className="text-xs text-white/70">Dynamic optimization engine with global adjustments</p>
          </div>
        </div>
